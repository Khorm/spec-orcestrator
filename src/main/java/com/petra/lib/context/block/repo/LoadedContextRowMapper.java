...
public class LoadedContextRowMapper implements RowMapper<ContextEntity>, ResultSetExtractor<ContextEntity> {
    ...
    private ContextEntity getLoadedContext(ResultSet rs) throws SQLException, JsonProcessingException {
        ...
        List<ValueDto> producerValues = oj.readValue(producerValuesStr, oj.getTypeFactory().constructCollectionType(List.class, ValueDto.class));

        List<ValueDto> outValues = null;
        if (valuesStr != null){
            outValues = oj.readValue(valuesStr, oj.getTypeFactory().constructCollectionType(List.class, ValueDto.class));
        }
